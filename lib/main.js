...
function copyResources(root, watchFile, target) {
	...
	if (fs.existsSync(src)) {
	    ...
	    console.log("ERROR".red+": Unable to copy "+src.underline+
...
function processSlides(root, watchFile) {
	...
	for(var pi=0;pi<pages.length;pi++) {
		...
		if (insert instanceof Array) {
		    ...
		    console.log("ERROR".red+": For page "+page.underline+
	...
	for(var pi=0;pi<pages.length;pi++) {
		...
		if (page.slice(-5)===".html" ||
		    ...
		    page.slice(-3)===".md" ||
		    page.slice(-5)===".jade") {
		    console.log("Warning".yellow+": File "+page.underline+
		...
		if (efile) {
			...
			/* TODO: Doesn't work properly for files in directories */
			console.log("ERROR".red+": slide "+page.underline+
				    " extends from non-existent template "+efile);
			template = swig.compile(text, { filename: page });
		    } else {
			watchFile(efile); // Watch the templates too
		    }
		    var ttext = '{% extends "'+efile+'" %}\n'+text
		    template = swig.compile(ttext, { filename: page });
		} else {
		    template = swig.compile(text, { filename: page });
		}
		...
		console.log("ERROR".red+": Unable to process page "+
