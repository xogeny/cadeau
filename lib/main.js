...
function copyResources(root, watchFile, target) {
	...
	if (fs.existsSync(src)) {
	    ...
	    console.log("ERROR".red+": Unable to copy "+src+" since it doesn't exist");
	};
    }
...
function processSlides(root, watchFile) {
	...
	for(var pi=0;pi<pages.length;pi++) {
		...
		if (insert instanceof Array) {
		    ...
		    console.log("ERROR".red+": For page "+page+", expected list of slide files, got "+insert+" instead");
		}
	    }
	}
	...
	for(var pi=0;pi<pages.length;pi++) {
		...
		if (page.slice(-5)===".html" ||
		    ...
		    page.slice(-3)===".md" ||
		    page.slice(-5)===".jade") {
		    console.log("Warning".yellow+": File "+page+" does not exist, treating as literal");
		}
		...
		if (efile) {
			...
			console.log("ERROR".red+": slide "+page+" extends from non-existent template "+efile);
		...
		console.log("ERROR".red+": Unable to process page "+page+", unknown type");
	    }
